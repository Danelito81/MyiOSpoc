...
struct MonitorView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            Text("Skyddsläge").font(.title.bold())
            Circle()
            ...
            Button(isRunning ? "Stoppa skydd" : "Starta skydd") { isRunning ? stop() : start() }
        ...
        .onChange(of: risk) { newRisk in
            if newRisk >= 0.85 { Haptics.warnStrong(); Voice.warn() }
            else if newRisk >= 0.65 { Haptics.warnSoft() }
        }
    }
    private var riskLabel: String { risk >= 0.85 ? "HÖG" : (risk >= 0.65 ? "MED" : "LÅG") }
    private func start() {
        do {
            try AudioSessionManager().configureForMonitoring()
            audio.onChunk = { pcm in
                let text = speech.transcribeChunk(pcm: pcm)
                if !text.isEmpty {
                    transcriptPreview = String((transcriptPreview + " " + text).suffix(160))
                    let res = detector.append(text)
                    DispatchQueue.main.async { risk = res.risk }
                }
            }
            try audio.start(); isRunning = true
        } catch { print("Start error: \(error)") }
    }
    private func stop() { audio.stop(); detector.reset(); risk = 0; isRunning = false }
}
